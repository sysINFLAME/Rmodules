...
{
	console.log("dropOntoCategorySelectiondropOntoCategorySelectiondropOntoCategorySelection")
	var targetdiv=this.el;
...
function getAllChildNodes(child,targetdiv){
	...
	console.log(child);
	if (!child.isLoaded() && child.leaf == false)
	{
		console.log("IF")
		child.reload(function(){
//			analysisConcept = dropOntoCategorySelection2(source, e, data, targetdiv);
			console.log("RELOADING");
			getAllChildNodes(child,targetdiv)
		});

	}
	else {
		console.log("child.node.childNodes.length: " + child.childNodes.length)
		for ( var i = 0; i<child.childNodes.length; i++)
		{
			console.log("for " + i)
			var current_child=child.childNodes[i];
			//Grab the child node.
...
function dropOntoCategorySelection2(source, e, data, targetdiv)
{
	...
	if(data.node.leaf==false)
	{
//		getAllChildNodes(data.node,targetdiv);
		//Keep track of whether the folder has any leaves.
		var foundLeafNode = false
